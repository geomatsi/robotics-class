...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    # algorithm is similar to Vitterbi decoding :)
    ...
    while 1:
        ...
        for i in range(len(path)):
            if path[i][0][0] == goal[0] and path[i][0][1] == goal[1]:
                found = True
                best = i
        ...
        for i in range(len(path)):
            ...
            for j in range(len(move)):
                ...
                if move[j][0] != 0:
                    c1 = c + lane_change_cost + 1.0/road[y][x]
                else:
                    c1 = c + 1.0/road[y][x]

                w1 = w + [[y1, x1]]

                # before adding the new position to path_next check if we already
                # have that new position with lower cost

                found = False

                for k in range(len(path_next)):
                    if x1 == path_next[k][0][1] and y1 == path_next[k][0][0]:
