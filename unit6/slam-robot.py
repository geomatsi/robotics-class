...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    Omega_x = matrix()
    Omega_x.zero(N + num_landmarks, N + num_landmarks)

    Omega_y = matrix()
    Omega_y.zero(N + num_landmarks, N + num_landmarks)

    Xi_x = matrix()
    Xi_x.zero(N + num_landmarks, 1)

    Xi_y = matrix()
    Xi_y.zero(N + num_landmarks, 1)

    # constraints for initial location (NB: no noise)
    Omega_x.value[0][0] +=  1.0
    Omega_y.value[0][0] +=  1.0

    # start position: center of the world
    Xi_x.value[0][0] += world_size/2.0
    Xi_y.value[0][0] += world_size/2.0

    for i in range(len(data)):

        # constraints for motion
        Omega_x.value[i][i]     +=  1/motion_noise
        Omega_x.value[i][i+1]   += -1/motion_noise
        Omega_x.value[i+1][i]   += -1/motion_noise
        Omega_x.value[i+1][i+1] +=  1/motion_noise

        Omega_y.value[i][i]     +=  1/motion_noise
        Omega_y.value[i][i+1]   += -1/motion_noise
        Omega_y.value[i+1][i]   += -1/motion_noise
        Omega_y.value[i+1][i+1] +=  1/motion_noise

        Xi_x.value[i][0]   += -data[i][1][0]/motion_noise
        Xi_x.value[i+1][0] +=  data[i][1][0]/motion_noise

        Xi_y.value[i][0]   += -data[i][1][1]/motion_noise
        Xi_y.value[i+1][0] +=  data[i][1][1]/motion_noise
        ...
        for k in range(len(z)):
            ...
            Omega_x.value[i][i]                 +=  1/measurement_noise
            Omega_x.value[i][N+z[k][0]]         += -1/measurement_noise
            Omega_x.value[N+z[k][0]][i]         += -1/measurement_noise
            Omega_x.value[N+z[k][0]][N+z[k][0]] +=  1/measurement_noise

            Omega_y.value[i][i]                 +=  1/measurement_noise
            Omega_y.value[i][N+z[k][0]]         += -1/measurement_noise
            Omega_y.value[N+z[k][0]][i]         += -1/measurement_noise
            Omega_y.value[N+z[k][0]][N+z[k][0]] +=  1/measurement_noise

            Xi_x.value[i][0]         += -z[k][1]/measurement_noise
            Xi_x.value[N+z[k][0]][0] +=  z[k][1]/measurement_noise

            Xi_y.value[i][0]         += -z[k][2]/measurement_noise
            Xi_y.value[N+z[k][0]][0] +=  z[k][2]/measurement_noise
    ...
    for i in range(0, 2*(N + num_landmarks), 2):
        mu.value[i][0]   = mu_x.value[i/2][0]
        mu.value[i+1][0] = mu_y.value[i/2][0]
