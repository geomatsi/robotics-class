...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # expand matrix to integrate the next step

        Omega = Omega.expand(2*(2 + num_landmarks), 2*(2 + num_landmarks),
            [0, 1] + [x for x in range(4, 2*(2 + num_landmarks))],
            [0, 1] + [x for x in range(4, 2*(2 + num_landmarks))])

        Xi = Xi.expand(2*(2 + num_landmarks), 1,
                [0, 1] + [x for x in range(4, 2*(2 + num_landmarks))], [0])

        # integrate the next motion
        ...
        # integrate the next measurements
        ...
        # shrink matrices: leave only the latest motion

        A = Omega.take([0, 1], [x for x in range(2, 2*(2 + num_landmarks))])
        B = Omega.take([0, 1], [0, 1])
        _Omega = Omega.take([x for x in range(2, 2*(2 + num_landmarks))],
            [x for x in range(2, 2*(2 + num_landmarks))])

        C = Xi.take([0, 1], [0])
        _Xi = Xi.take([x for x in range(2, 2*(2 + num_landmarks))], [0])
...
# Run both versions of slam and print results for comparison
