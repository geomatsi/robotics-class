...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte_orig(self, radius):
        ...
        if (self.x <= c1x):
            cte = sqrt((self.x - c1x)**2 + (self.y - cy)**2) - radius

            a = (cos(self.orientation)*(self.x - c1x) +
                sin(self.orientation)*(self.y - cy))/sqrt((self.x -
                    c1x)**2 + (self.y - cy)**2)
        ...
        elif (self.x >= c1x) and (self.x <= c2x):
            if cos(self.orientation) > 0:
                cte = self.y - (cy + radius)
            else:
                cte = -(self.y - (cy - radius))

        else:
            cte = sqrt((self.x - c2x)**2 + (self.y - cy)**2) - radius

            a = (cos(self.orientation)*(self.x - c2x) +
                    sin(self.orientation)*(self.y - cy))/sqrt((self.x -
                        c2x)**2 + (self.y - cy)**2)
...
def twiddle(radius, tol = 0.2, debug = False): #Make this tolerance bigger if you are timing out!

    params = [0.0, 0.0, 0.0]
    dp = [1.0, 1.0, 1.0]
    best_err = run(params, radius)
